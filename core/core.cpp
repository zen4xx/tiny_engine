...
Tiny_engine::Tiny_engine(const char* applictaion_name, int width, int height, const char* title, bool use_default_shaders, GLFWmonitor* monitor, GLFWwindow* window){
    glfwInit();
    m_window = std::make_unique<Window>(width, height, title, monitor, window);
    m_renderer = std::make_unique<Renderer>(applictaion_name, use_default_shaders);
